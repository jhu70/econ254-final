...
c1_formula <- as.formula(paste("samegroup ~ difchoice1+frfamcl+dcfrfamcl"))
print(c1_formula)
```
...
```{r, message = FALSE}
library(margins)
c1_m<-margins(c1,data = find_data(c1, parent.frame()),variables = c("difchoice1","frfamcl","dcfrfamcl"),type = c("response","link"),vcov = stats::vcov(c1), vce = c("delta"), unit_ses = FALSE)
summary(c1_m)
```
...
c5m[is.num] <- lapply(c5m[is.num], round, 3)
